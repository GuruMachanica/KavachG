...
def analyze_incident_with_ai(incident_type: str, confidence: float = None, description: str = "") -> dict:
    """Agentic AI triage: produces structured root-cause, OSHA standard, and remediation protocol."""
    ...
    elif "restrict" in key or "zone" in key:
        rule = OSHA_STANDARDS["restricted"]
    else:
        rule = {
            "standard": "General Duty Clause (OSH Act Section 5(a)(1))",
            "title": "General Workplace Hazard Prevention",
            "criticality": "Medium",
            "guidelines": "Maintain environment free from recognized hazards causing or likely to cause physical harm.",
            "actions": ["Conduct site supervisor inspection", "Document corrective note"]
        }

    return {
        "timestamp": datetime.utcnow().isoformat(),
        "incident_type": incident_type,
        "confidence_level": f"{int((confidence or 0.85) * 100)}%",
        "osha_reference": rule["standard"],
        "osha_title": rule["title"],
        "criticality": rule["criticality"],
        "regulatory_requirement": rule["guidelines"],
        "recommended_immediate_actions": rule["actions"],
        "ai_summary": f"Automated Safety Agent Assessment: Incident '{incident_type}' detected. Priority rating: {rule['criticality']}. Compliance mandate under {rule['standard']} applies."
    }


def generate_daily_safety_briefing() -> dict:
    """Generates an executive AI safety briefing summarizing total incidents, risk hotspots, and recommendations."""
    ...
    stats_summary = {}
    for itype, status, count in breakdown:
        stats_summary[f"{itype}_{status}"] = count
...
def ask_safety_copilot(query: str) -> dict:
    ...
    if "how many" in q or "count" in q or "today" in q or "status" in q:
        ...
        rule = OSHA_STANDARDS["ppe"]
        response = f"**PPE Compliance Policy ({rule['standard']})**: All workers inside machinery and active construction sectors must wear ANSI-compliant hardhats and high-visibility vests. Supervisors are notified automatically upon 5 seconds of sustained violation."
